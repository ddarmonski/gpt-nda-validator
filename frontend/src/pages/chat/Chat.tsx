...
const Chat = () => {
    ...
    const [selectedGPTVersion, setSelectedGPTVersion] = useState<string>("GPT 4.0"); 

    const [selectedItems, setSelectedItems] = useState<IDropdownOption[]>([]);
    const [selectedTemplates, setSelectedTemplates] = useState<IDropdownOption[]>([]);  
   /* const items: IDropdownOption[] = [  
    ...
    const [items, setItems] = useState<IDropdownOption[]>([]);
    const [nda_templates, setTemplates] = useState<IDropdownOption[]>([]);

    //Get NDA agreement
    useEffect(() => {  
        axios.get('/get_files')  // replace with your Flask backend URL if not on the same domain  
          .then(response => {  
            const dropdownOptions = response.data.map((path:string, index:number) => ({  
              key: index,  
              text: path,  
            }));  
          ...
          .catch(error => {  
            console.error('Failed to fetch filepaths:', error);  
          });  
      }, []); 

      //Get NDA Template
      useEffect(() => {  
        axios.get('/get_nda_templates')  // replace with your Flask backend URL if not on the same domain  
          .then(response => {  
            const dropdownOptions = response.data.map((path:string, index:number) => ({  
              key: index,  
              text: path,  
            }));  
            setTemplates(dropdownOptions);  
          })  
          .catch(error => {  
            console.error('Failed to fetch filepaths:', error);  
          });  
    ...
    const onChange = (event: React.FormEvent<HTMLDivElement>, item?: IDropdownOption): void => {    
        if (item) {  
            const updatedSelectedItems = [...selectedItems]; // creating a copy of selectedItems  
            if (item.selected) {  
                updatedSelectedItems.push(item);  
            } else {  
                const itemIndex = updatedSelectedItems.findIndex(selectedItem => selectedItem.key === item.key);  
                if (itemIndex > -1) {  
    ...
    const onChange_templates = (event: React.FormEvent<HTMLDivElement>, item?: IDropdownOption): void => {    
        if (item) {  
            const updatedSelectedTemplates = [...selectedTemplates]; // creating a copy of selectedTemplates  
            if (item.selected) {  
                updatedSelectedTemplates.push(item);  
            } else {  
                const itemIndex = updatedSelectedTemplates.findIndex(selectedItem => selectedItem.key === item.key);  
                if (itemIndex > -1) {  
                    updatedSelectedTemplates.splice(itemIndex, 1);  
                }  
            }  
    ...
    const gptVersionOptions = [  
        { key: 'GPT 3.5', text: 'GPT 3.5', disabled: true },  
    ...
    const makeApiRequestWithoutCosmosDB = async (question: string, conversationId?: string) => {
        ...
        const request: ConversationRequest = {
            ...
            selectedItems: selectedItems.map(item => item.text).join(','),  
            selectedTemplates: selectedTemplates.map(item => item.text).join(','),
        };
        ...
        <div className={styles.container} role="main">
            ...
            {showAuthMessage ? (
                ...
                <Stack horizontal className={styles.chatRoot}>
                    ...
                    <Dropdown   
                                    label="Select 3rd party NDA for review"  
                                    selectedKeys={selectedItems.map(i => i.key as string)} 
                                    onChange={onChange}  
                                    options={items}  
                                    multiSelect  
                                /> 
                    ...
                    <Dropdown   
                                    label="Select NDA template"  
                                    selectedKeys={selectedTemplates.map(i => i.key as string)} 
                                    onChange={onChange_templates}  
                                    options={nda_templates}  
                                    multiSelect  
                                /> 
                        ...
                        {!messages || messages.length < 1 ? (
                            <Stack className={styles.chatEmptyState}>
                                <img
                                    ...
                                    aria-hidden="true"
                                />
                                <h1 className={styles.chatEmptyStateTitle}>AI NDA Validator</h1>
                                <h2 className={styles.chatEmptyStateSubtitle}>Powered by Phoenix GPT</h2>
                            </Stack>
                        ) : (
